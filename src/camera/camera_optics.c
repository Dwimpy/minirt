...
/*   Updated: 2023/07/26 21:38:53 by arobu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera	cam_setup(t_cam_params params)
{
	...
	cam_set_orientation(&cam, eul_orient(0.0, 0.0, 0.0));
	cam_compute_matrix(&cam);
	cam_compute_optics(&cam);
	return (cam);
}

void	cam_compute_matrix(
	t_camera *cam)
{
	...
	rotate = tf_rotate(eul_cam_orient(cam->orientation));

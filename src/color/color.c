...
/*   Updated: 2023/07/13 18:14:46 by arobu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color
color_scale(double scale, t_color color) {
	return ((t_color) {color.r * scale,color.g * scale, color.b * scale});
}

t_color
color_dot(t_color c1, t_color c2) {
...
t_color	color_blend(t_ray ray, t_list *list, int depth, t_sampler *sampler)
{
	...
	if (hit_anything) {
		...
		if ((hit_rec.material->scatter(
			(t_scatter_params) {&ray,&hit_rec,&attenuation,&scattered,hit_rec.material->refraction_index},sampler)))
	...
	final_color = color_scale(1.0,(t_color) {1.0,1.0,1.0});
	final_color =
		color_add(final_color,color_scale(t,(t_color) {0.5,0.7,1.0}));
	return (final_color);
}

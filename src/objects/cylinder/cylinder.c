...
t_cylinder *create_cylinder(t_cylinder_data data, t_material material) {
	...
	cyl_info = shape_type_create((t_shape_info) {
		cylinder_get_name_override, cylinder_hit, cylinder_destroy_override});
	...
	data.lower = create_disk(
		(t_disk_data) {data.center, vec_scale(-1, data.axis), data.radius},
		material);
	data.upper = create_disk(
		(t_disk_data) {vec_add(data.center, vec_scale(data.height, data.axis)),
					   data.axis, data.radius},
		material);
...
t_vec3 cyl_normal_at(t_cylinder *cyl, t_ray *ray, t_vec3 oc, t_hit_rec *hit) {
	...
	m = a * hit->t + b;
	return (vec_normalize(vec_sub(vec_sub(hit->isec_point, cyl->data->center),
